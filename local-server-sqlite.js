...
async function initDatabase() {
  try {
    ...
    console.log('Base de données SQLite locale initialisée avec succès');
    return true;
  } catch (error) {
...
app.get('/api/companies', authenticateToken, async (req, res) => {
  try {
    const companies = await db.all('SELECT * FROM companies WHERE userId = ?', [req.user.id]);
  ...
  try {
    ...
    res.json({
      ...
      invoices: invoices.map(invoice => ({
        ...
        client: JSON.parse(invoice.client),
